...
public class EresourceSQLTranslator extends AbstractSQLTranslator {
    ...
    public List<String> getInsertSQL(final Eresource er) {
        ...
        StringBuilder sb = new StringBuilder(this.getInsertInto()).append(INSERT_ERESOURCE)
                ...
                .append(apostrophize(er.getPrimaryType())).append(COMMA).append(er.isCore() ? "'Y'" : NULL)
                .append(COMMA).append(er.getYear() > 0 ? Integer.toString(er.getYear()) : NULL).append(COMMA)
                .append(itemCount[0]).append(COMMA).append(itemCount[1]).append(COMMA)
                .append(er.getDescription() != null ? "empty_clob()" : NULL).append(", empty_clob())");
        sql.add(sb.toString());
