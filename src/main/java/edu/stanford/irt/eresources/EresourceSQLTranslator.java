...
public class EresourceSQLTranslator extends AbstractSQLTranslator {
    ...
    public String getEresourceIdSQL(final Eresource er) {
        StringBuilder sb = new StringBuilder("SELECT ERESOURCE_ID FROM ")
                .append("ERESOURCE WHERE RECORD_ID = ")
                .append(er.getRecordId()).append(" AND RECORD_TYPE = '")
    ...
    private Collection<String> getInsertMeshSQL(final Eresource er) {
        ...
        for (String meshTerm : er.getMeshTerms()) {
            sb.setLength(0);
            sb.append("INSERT INTO MESH VALUES (")
            .append("ERESOURCE_ID_SEQ.CURRVAL,")
            .append(apostrophize(meshTerm))
            .append(END_PAREN);
            sql.add(sb.toString());
        }
        return sql;
