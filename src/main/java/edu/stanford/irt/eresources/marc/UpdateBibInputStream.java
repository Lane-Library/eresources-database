...
public class UpdateBibInputStream extends UpdateEresourceInputStream {
    ...
    private static final String QUERY = "SELECT bib_master.bib_id, " + "  mfhd_master.mfhd_id "
            + "FROM lmldb.bib_master, " + "  lmldb.bib_index, " + "  lmldb.mfhd_master, " + "  lmldb.bib_mfhd "
            ...
            + "AND mfhd_master.suppress_in_opac != 'Y' " + "AND bib_master.bib_id            IN " + "  (SELECT bib_id "
            + "  FROM lmldb.bib_master " + "  WHERE update_date > ? " + "  OR create_date    > ? " + "  UNION "
            + "  SELECT bib_mfhd.bib_id " + "  FROM lmldb.bib_mfhd, " + "    lmldb.mfhd_master "
            + "  WHERE mfhd_master.mfhd_id = bib_mfhd.mfhd_id " + "  AND (update_date          > ? "
            + "  OR create_date            > ?) " + "  UNION " + "  SELECT bib_item.bib_id "
            + "  FROM lmldb.bib_item, " + "    lmldb.item_status item_status_1 "
