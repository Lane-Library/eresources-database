...
public class SulMARCRecordEresourceProcessor extends AbstractEresourceProcessor {
    ...
    public boolean isInScope(final Record marcRecord) {
    ...
    public boolean isDuplicate(final Record marcRecord) {
        ...
        for (String isbn : MARCRecordSupport.getSubfieldData(marcRecord, "020", "az").map(String::trim)
        ...
        for (String issn : MARCRecordSupport.getSubfieldData(marcRecord, "022", "alz").map(String::trim)
                .map(TextHelper::cleanIsxn).filter((final String s) -> !s.isEmpty()).collect(Collectors.toSet())) {
