...
public class PrintBibInputStream extends BibInputStream {
    ...
    private static final String QUERY =
          ...
          + "  SELECT BIB_ID " 
          + "  FROM LMLDB.BIB_TEXT " 
          + "  WHERE (REGEXP_LIKE(BEGIN_PUB_DATE,'^\\d{4}$') " 
          + "  AND BEGIN_PUB_DATE >= TO_CHAR(SYSDATE - (10 * 365),'YYYY')) " 
          + "  OR (REGEXP_LIKE(END_PUB_DATE,'^\\d{4}$') " 
          + "  AND END_PUB_DATE >= TO_CHAR(SYSDATE - (10 * 365),'YYYY')) " 
