...
public class EresourceSQLTranslator extends AbstractSQLTranslator {
    ...
    public EresourceSQLTranslator(final VersionSQLTranslator versionTranslator) {
        this.versionTranslator = versionTranslator;
    }

    public String getEresourceIdSQL(final Eresource er) {
        StringBuilder sb = new StringBuilder("SELECT ERESOURCE_ID FROM ").append("ERESOURCE WHERE RECORD_ID = ")
                .append(er.getRecordId()).append(" AND RECORD_TYPE = '").append(er.getRecordType()).append(APOS);
        return sb.toString();
    }
