...
<xsl:template match="table[@id='pageContent_GridView1']">
	...
	<eresource>
			<xsl:attribute name="id"><xsl:value-of select="td[1]" /></xsl:attribute>
			...
			<year><xsl:value-of select="replace(./td[6],'.*/(\d{4})','$1')"/></year>
			<version>
				<link>
					<url>
			...
			<xsl:if test="td[3]/text() != '' ">
				<publicationAuthorsText><xsl:value-of select="./td[3]"/></publicationAuthorsText>
			</xsl:if>	
		</eresource>
